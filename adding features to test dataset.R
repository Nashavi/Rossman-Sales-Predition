t <- read.csv("Datasets/test.csv")
s <- read.csv("Datasets/Stores.csv")
s<- s[,-1]

t <- merge(t,s,by="Store",all.x = TRUE)
rm(s)

t$Date <- as.Date(t$Date,'%Y-%m-%d')
t <- merge(t,calendar,by="Date",all.x = TRUE)

#now just remove duplicated columns from merge
t = t[,-which(names(t) %in% c("DayOfWeek"))] 
...
write.csv(t,"Datasets/test2.csv")

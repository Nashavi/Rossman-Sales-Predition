...
d$dayofWeek <- factor(dayofWeek,levels=unique(d$dayofWeek))
...
mod <- lm(Customers~StoreType++Assortment+Promo+month+year+dayofWeek+Open,data=d)
plot(mod)
...
ggplot(d,aes(factor(weekday),Sales,fill=weekday))+bp+noleg+scale_x_discrete(labels=c("0","1","2","3","4","5","6"))
...
tsales <- as.data.frame(aggregate(d$Sales,by=list(d$month),FUN=median))
...
ggplot(d,aes(factor(StoreType),Sales,fill=StoreType))+geom_boxplot()+facet_wrap(~weekday)
ggplot(d,aes(factor(Promo),Sales,fill=Promo))+bp+facet_wrap(~weekday)
...
ggplot(d,aes(factor(StoreType),Customers))+geom_boxplot()

### some modeling based on EDA 
mod <- lm(Sales~Customers+StoreType+Open+month+dayofWeek+Promo+Assortment,d)
summary(mod)
plot(mod)
...
step.model = step(null.model,scope=list(upper=full.model),data=d,direction="both")
